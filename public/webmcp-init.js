...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (content) {
    ...
    if (header) {
      ...
      var promptText = 'I want to connect to glmu.cc via the @jason.today/webmcp package. First check the package\'s CLI interface (run npx @jason.today/webmcp --help) to see how to register it as an MCP server for my client, then set it up accordingly. Once it\'s registered, use the generate-token tool to create a connection token for glmu.cc and give me the raw token. Once connected, you can use these tools on this site: get_scope_details (infrastructure, ai, training engagements), get_credentials (certifications), get_contact (email/social), get_competencies (technical stack). You can also read page://current or section://{elementId}.';
